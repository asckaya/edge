...
**关键参数：**
- `type`：`*-full` / `*-dual` / `*-minimal` (其中 `*` 为 `mihomo` / `stash` / `sing-box`)
...
**模板选择逻辑：**
```typescript
const isMinimal = configType.endsWith('-minimal');
const isDual    = configType.endsWith('-dual');

const useMinimalTemplates = isMinimal || isDual;
const tplGroupsHeader  = useMinimalTemplates ? configMihomoMinimalGroupsHeader : isStash ? configStashGroupsHeader : configMihomoGroupsHeader;
const tplGroupsMid     = useMinimalTemplates ? configMihomoMinimalGroupsMid    : isStash ? configStashGroupsMid    : configMihomoGroupsMid;
...
- **`minimal/`**：极简黑名单版。包含 `groups.ts` (4 个基础策略组 + 7 个区域组)、`rule-providers.ts` 和 `rules.ts` (黑名单规则)。
...
- **Full 版**：全量规则 + 全量场景分组（60+ 策略组）。
- **Dual 版 (推荐)**：全量规则 + 极致精简分组。将 `🧲 BT/PT` 强制导向 `DIRECT`（防止封号），其余所有国外场景重定向到 `🚀 节点选择`。保留 `🔒 国内服务` 分组，其余策略组全部剔除。
- **Minimal 版**：极简黑名单版。仅核心国外服务规则走 `🚀 节点选择`，其余兜底 `DIRECT`。
